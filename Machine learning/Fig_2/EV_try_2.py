...
X_train, X_test, y_train, y_test = train_test_split(x, y_label, test_size=0.4, stratify=y_label, random_state=42)
...
ovr_clf = OneVsRestClassifier(XGBClassifier(use_label_encoder=False, eval_metric='logloss', random_state=42, reg_lambda=1.0, reg_alpha=0.5)) #max_depth=3, min_child_weight=1, gamma=0, subsample=0.8, colsample_bytree=0.8, reg_lambda=1.0, reg_alpha=0.5))
...
for j, feature_name in enumerate(feature_columns):
    ...
    ovr_clf = OneVsRestClassifier(XGBClassifier(use_label_encoder=False, eval_metric='logloss', random_state=42))
