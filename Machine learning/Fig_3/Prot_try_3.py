...
for j, feature_name in enumerate(feature_columns):
    ...
    olr.fit(X_train_feature, y_train_smote)
    
    # Predicting class labels for the test set
    y_pred = olr.predict(X_test_feature)
    
    # Predicting probabilities for ROC AUC
    y_proba = olr.predict_proba(X_test_feature)
    ...
    cv_results = cross_validate(olr, X_train_smote, y_train_smote, cv=cv, scoring=scoring)
    print(cv_results)
