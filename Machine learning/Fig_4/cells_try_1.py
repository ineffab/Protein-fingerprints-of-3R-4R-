...
#load the data
data = pd.read_excel("Cells_2.xlsx", header=0)
...
#ovr_clf_1 = XGBClassifier(objective = 'multi:softprob', eval_metric='mlogloss', random_state=42, max_depth=3, min_child_weight=1, gamma=0, subsample=0.8, colsample_bytree=0.8, reg_lambda=1.0, reg_alpha=0.5) #max_depth=3, min_child_weight=1, gamma=0, subsample=0.8, colsample_bytree=0.8, reg_lambda=1.0, reg_alpha=0.5))
...
ovr_clf_1.fit(X_train_smote, y_train_smote)

# Make predictions with the test set
y_pred = ovr_clf_1.predict(X_test)  # These are class labels, not probabilities
...
for i, class_name in enumerate(le.classes_):
    ...
    y_score = ovr_clf_1.predict_proba(X_test)[:, i]  # Get probabilities for the current class
...
for i, feature_name in enumerate(data.columns[1:]):  # Adjust based on your dataset's feature columns
    ...
    accuracy_scores = cross_val_score(ovr_clf_1, X_train_feature, y_train_smote, cv=cv, scoring= accuracy_macro_scorer)
    # Store the average score for the feature
    accuracy[feature_name] = np.mean(accuracy_scores)

    # Perform cross-validation for the current feature
    precision_scores = cross_val_score(ovr_clf_1, X_train_feature, y_train_smote, cv=cv, scoring=precision_macro_scorer)
    # Store the average score for the feature
    precision[feature_name] = np.mean(precision_scores)

    # Perform cross-validation for the current feature
    recall_scores = cross_val_score(ovr_clf_1, X_train_feature, y_train_smote, cv=cv, scoring=recall_macro_scorer)
    # Store the average score for the feature
    recall[feature_name] = np.mean(recall_scores)

    # Perform cross-validation for the current feature
    f1_scores = cross_val_score(ovr_clf_1, X_train_feature, y_train_smote, cv=cv, scoring=f1_macro_scorer)
    # Store the average score for the feature
...
for j, feature_name in enumerate(feature_columns):
    ...
    #ovr_clf = XGBClassifier(objective = 'multi:softprob', eval_metric='mlogloss', random_state=42, max_depth=3, min_child_weight=1, gamma=0, subsample=0.8, colsample_bytree=0.8, reg_lambda=1.0, reg_alpha=0.5)
    ...
    ovr_clf.fit(X_train_feature, y_train_smote)
    
    # Predicting class labels for the test set
    y_pred = ovr_clf.predict(X_test_feature)
    
    # Predicting probabilities for ROC AUC
    y_proba = ovr_clf.predict_proba(X_test_feature)
    ...
    #    y_test_bin = label_binarize(y_test, classes=range(n_classes))[:, i]
    #    y_score = y_proba[:, i]
    #    auc_score = roc_auc_score(y_test_bin, y_score)
    #    fpr, tpr, _ = roc_curve(y_test_bin, y_score)
    #    
    #    plt.figure()
    #    plt.plot(fpr, tpr, label=f'Feature {feature_name} - Class {i} (AUC = {auc_score:.2f})')
    #    plt.plot([0, 1], [0, 1], 'k--')
    #    plt.xlabel('False Positive Rate')
    #    plt.ylabel('True Positive Rate')
    #    plt.title(f'ROC Curve for {feature_name} - Class {i}')
    #    plt.legend(loc='lower right')
    #    plt.show()


df = pd.read_excel("F1_ovr.xlsx")
...
df_1 = pd.read_excel("F1_ovr_1.xlsx")

df_1.set_index('Feature_Name', inplace = True) 

# Assuming 'df' is your DataFrame prepared as above
sns.heatmap(df_1, annot=True, cmap='coolwarm', fmt=".2f")
plt.title('F1-Scores for Each Feature Across Classes')
plt.ylabel('Feature')
plt.xlabel('Class')
plt.show()
